...
public class Instruction
	...
	*/
    ...
    protected boolean srMatchesCCInfo(int sr) {
        if (brccInfo != null) {
            for (CCInfo ccInfo : brccInfo) {
                if ((sr & ccInfo.mask) == ccInfo.check)
                    return true;
            }
        }
        return false;
    ...
    public String getRepr(int[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            String a;
            if (opndInfo[i].inlineOpndKind == OPND_INL_REG) {
